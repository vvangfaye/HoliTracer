...
def process_mask_file(args):
    ...
    SIMPLIFY_VALUE = 1.0
    AREA_FLITER_VALUE = 100

    mask_filename, masks_dir, image_id_map, image_width, image_height, category_id = (
        args
    )
    annotations = []
    images = []
...
def masks_to_coco_label(
    masks_dir, save_json_path, image_width, image_height, num_processes=1
):
    """
    ...
    # 定义类别（假设只有一个类别）
    category_id = 1
    category_name = "water"  # 根据实际情况替换类别名称
    ...
    image_files = [f.replace(".png", ".jpg") for f in mask_files]
    image_id = range(len(image_files))

    image_id_map = dict(zip(image_files, image_id))
